...
public final class StaffBanCommand implements CommandExecutor {

    private final StaffCore plugin;

    public StaffBanCommand(StaffCore plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!Permissions.has(sender, "staffcore.punish.ban")) {
        ...
        if (args.length < 2) {
            plugin.messages().send(sender, "ban-usage");
            return true;
        }
        ...
        plugin.punishmentManager().ban(sender, target, targetName, reason, null);
        plugin.messages().send(sender, "punishment-ban-sent", Map.of("player", targetName, "duration", "permanent"));
        return true;
    }
}
