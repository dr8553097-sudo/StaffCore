...
public final class TempBanCommand implements CommandExecutor {
    ...
    public TempBanCommand(StaffCore plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!Permissions.has(sender, "staffcore.punish.tempban")) {
        ...
        if (args.length < 3) {
            plugin.messages().send(sender, "tempban-usage");
            return true;
        }
        ...
        plugin.punishmentManager().ban(sender, target, targetName, reason, duration);
        plugin.messages().send(sender, "punishment-ban-sent", Map.of("player", targetName, "duration", durationText));
        return true;
    }
}
